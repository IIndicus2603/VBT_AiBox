...
import {
  $, $$, S, API, FILTERS, hms, pad, fixPct, nice, mask, row,
...
function toast({sev, title, sub, name, kind, img, onSkip, color, ev}) {
  ...
  box.style.borderTopColor = col;
  box.innerHTML =
    '<div class="t-h"><span class="t-dot"></span><span class="t-sev"></span>' +
      '<div class="grow"></div><span class="t-older" hidden></span>' +
      '<span class="t-time"></span><button class="t-x">&#10005;</button></div>' +
    '<div class="t-body"><div class="t-ttl"></div><div class="t-sub"></div>' +
      '<div class="t-acts"><button class="go">Xem camera</button>' +
      '<button class="skip">Bỏ qua</button></div></div>';
  box.querySelector('.t-dot').style.background = col;
  const sevEl = box.querySelector('.t-sev');
  ...
  box.querySelector('.go').style.background =
    `linear-gradient(165deg,${col} 0%,${col} 45%,rgba(0,0,0,.28) 100%)`;
  ...
  // Dau X chi dong popup, vien van nhay — hai hanh vi khac nhau co y.
  ...
  if (img) {
    ...
    lb.className = 't-cam';
    lb.textContent = sub || name || '';
    // THU TU QUAN TRONG: chen vao DOM va gan onerror TRUOC khi gan src.
    shot.append(im, Object.assign(document.createElement('div'), {className: 'lines'}), lb);
    box.querySelector('.t-body').before(shot);
  ...
  stackToasts();
  if (kind === 'ok') setTimeout(kill, 6000);
  return kill;
}

/** Xep tang: toast moi nhat (cuoi DOM) nam tren cung, cac cai cu lui ve sau —
 *  dich len 30px, thu nho 1.5%, mo dan. Giong design. */
function stackToasts() {
  const list = [...$('#toasts').children];
  list.forEach((el, i) => {
    const depth = list.length - 1 - i;          // 0 = moi nhat
...
async function scanNetwork() {
  ...
  try {
    ...
    box.replaceChildren(...devs.map(d => {
      ...
      el.className = 'd-row';
      el.innerHTML = '<span class="d-ip"></span><span class="d-mf"></span>' +
        '<button class="btn d-add">Thêm</button>';
      el.querySelector('.d-ip').textContent = d.addr || d.ip;
      el.querySelector('.d-mf').textContent = d.manufacturer || '—';
...
async function loadCams() {
  ...
  try {
    ...
    if (j.code !== 0) throw new Error(j.msg || 'code ' + j.code);
    const cams = j.data || [];
    const on = cams.filter(c => c.status === 1).length;
    const tot = cams.reduce((n, c) => n + (c.algos || []).length, 0);
    $('#camSub').textContent = cams.length + ' camera · ' + on + ' online · ' +
      tot + ' thuật toán đang chạy';
...
function camRow(c) {
  ...
  el.innerHTML =
    ...
    '<div class="c-act">' +
      icoHtml('gold', 'Cấu hình AI', SVG_GEAR) +
  ...
  const tags = el.querySelector('.c-algos');
  if (!algos.length) tags.innerHTML = '<span class="c-none">chưa bật AI</span>';
  else {
    ...
    if (algos.length > 4) {
      ...
      s.className = 'c-none';
      s.textContent = '+' + (algos.length - 4);
      tags.append(s);
    }
  }
  const [cfg, view, del] = el.querySelectorAll('.ico');
...
function paintLogFilter(cats, all) {
  ...
  list.replaceChildren(...cats.map(c => {
    ...
    b.innerHTML = '<span class="al-box"></span><span class="dot"></span>' +
      '<span class="l"></span><span class="k"></span>';
...
startAlarms(ev => {
  ...
  const cam = camOf(ev);
  toast({
    sev: 'PHÁT HIỆN', kind: 'err',
    title: algoName(ev.algo_model, ev.algo_name) || ev.label,
    sub: [ev.channel_name || cam, hms(new Date((ev.ts || 0) * 1000))].filter(Boolean).join(' · '),
    name: cam && S.api[cam] ? cam : null,      // chi mo detail khi stream co that
    img: imgOf(ev),
    ev,                                        // co video_url -> nut thanh "Xem lai"

    onSkip: () => cam && clearAlarm(cam),      // "Bo qua" = tat luon vien nhay
  });
  markTiles();
  if (S.view === 'log') { loadLog(); unread = 0; }
  else bumpUnread();
...
/** Thanh lich su canh bao duoi stream o trang chi tiet. */
export function paintHistory() {
  ...
  if (!list.length) {
    wrap.innerHTML = '<span class="h-none">Chưa có cảnh báo nào từ AI box cho camera này</span>';
    return;
  }
...
$('#cnTest').onclick = async () => {
  ...
  try {
    ...
    const d = j.data || {};
    cnState('OK · ' + (d.device_name || d.model || '') + ' · SN ' + (d.device_sn || '—'), 'var(--ok)');
  } catch (e) {
    cnState(e.message, 'var(--err2)');
  }
};
...
$('#cnSync').onclick = async () => {
  ...
  try {
    ...
    if (j.code !== 0) return cnState(j.msg || 'Lỗi ' + j.code, 'var(--err2)');
    cnState('Đã đồng bộ ' + (j.added || 0) + ' luồng mới / ' + (j.total || 0) + ' channel',
            'var(--ok)');
    await refresh();
  } catch (e) {
    cnState(e.message, 'var(--err2)');
...
hooks.onView = view => {
  ...
  if (view === 'live') loadAreaOn().then(paintAreaBadge);
  if (view === 'detail') { paintHistory(); paintDetailAlgos(); }
  if (view === 'cam') loadCams();
  if (view === 'cfg') loadAlgos();
  if (view === 'log') { loadLog(); unread = 0; $('#navUnread').hidden = true; }
};
