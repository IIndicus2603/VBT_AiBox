...
const at = p => location.port === String(p) ? '' : `http://${H}:${p}/`;
...
export function paintTile(name) {
  ...
  t.box.querySelector('.scan').hidden = t.state !== 'live';
}
...
export function drawGrid() {
  ...
  $('#camCount').textContent = `${names.length}/${S.order.length} luồng RTSP`;
  const pg = $('#pager');
  pg.hidden = pages < 2;
  if (pages > 1) {
    ...
    $('#pgRange').textContent =
      `Luồng ${S.page * size + 1}–${Math.min(names.length, (S.page + 1) * size)} / ${names.length}`;
  }
}
